...
## 自动检测本机服务（Auto-detect localhost servers） {#auto-detect-localhost-servers}
...
## 手动附加连接（Attach manually） {#attach-manually}
...
该弹窗目前不支持带方括号的 IPv6 端点输入。若 Ante 探测 `/health` 和 `/v1/models` 失败，会在离线模式对话框中显示连接错误。

## 模型加载期间接入（Attaching while the model loads） {#attaching-while-the-model-loads}
...
## Docker 启动示例（Docker example） {#docker-example}
...
## 断开连接行为（Detach behavior） {#detach-behavior}
...
## 相关环境变量（Related environment variables） {#related-environment-variables}
