...
## 内置提供商列表（Built-in providers） {#built-in-providers}
...
## 各提供商的模型清单（Models by provider） {#models-by-provider}
...
| `claude-fable-5` | 针对高要求推理的最强能力 | 1M |
...
#### OpenAI 传输协议（OpenAI transports） {#openai-transports}
...
| `meta/muse-spark-1.2` | Muse Spark 1.2 多模态智能体推理模型 | 1.05M |
...
`openrouter-anthropic` 通过 Anthropic Messages 协议提供 Claude 家族：`anthropic/claude-haiku-4.5`（200K 上下文），以及 `anthropic/claude-sonnet-5`、`anthropic/claude-opus-5`、`anthropic/claude-opus-5-fast`、`anthropic/claude-fable-5`（1M 上下文）。
...
| 模型 ID | 说明 | 上下文大小 |
|---|---|---|
| `gemini-3.5-flash` | Gemini 3.5 Flash，适用于智能体工作流 | 1M |
...
| `claude-sonnet-4-6` | 快速且高能力 | 200K |
| `claude-opus-4-8` | 最强能力，推理深入 | 200K |
| `claude-fable-5` | 最强大的 Claude 模型 | 1M |
...
## `catalog.json` 配置规范（catalog.json schema） {#catalogjson-schema}
...
### 提供商字段（Provider fields） {#provider-fields}
...
| `stream_idle_timeout_secs` | — | 流式调用在超时报错前允许的最长无可用事件时间（秒）。涵盖等待首个事件和后续事件间隔，跨 SSE 和 WebSocket 均有效。默认 `300` 秒。慢速本地模型或代理端点可适当调大 |
...
针对内置提供商，仅需填写希望修改的字段。将可选字段设置为 `null` 可清除预设值：
...
### 额外请求字段（Extra request fields） {#extra-request-fields}
...
保留关键字在加载目录时会被自动忽略并在启动提示中告警，其余字段仍正常加载。若想修改这些参数，请使用[模型字段](#model-fields)或[环境变量覆盖](#environment-overrides)。

### 身份认证配置（Authentication） {#authentication}
...
使用 `oauth_preset` 接入内置 OAuth 流程（`anthropic`、`openai` 或 `antix`）：
...
| `temperature` | 采样温度。在 `AnthropicMessage` 提供商上，开启思考推理时必须为 `1` |
| `top_p` | Nucleus 采样截断阈值 |
| `top_k` | Top-k 采样截断阈值 |
| `max_tokens` | 单轮最大输出 Token 数。在需要显式思考预算的 `AnthropicMessage` 提供商上，设置思考等级时必须大于 1024 |
...
| `support_vision` | 是否支持图片输入。省略时 Ante 根据模型 ID 自动推断 |

顶层 `models` 可在解析阶段原地打补丁：
...
### 权重等级（Weight classes） {#weight-classes}

每个内置模型均带有 `weight_class`（`Feather`、`Middle` 或 `Heavy`），Ante 根据此属性自动进行智能选择：

- **默认模型** — 选择提供商但未指明具体模型时，默认选用其 **Heavy** 旗舰模型。
- **辅助任务调用** — 网页内容摘要（`WebFetch`）等轻量内部任务自动选用该提供商的 **Feather** 经济模型。

### 模型与提供商的解析仲裁规则（How a model and provider are chosen） {#how-a-model-and-provider-are-chosen}

解析模型 ID 时分层覆盖（后生效者胜出）：

1. **规范声明（Canonical spec）** — 目录中首个在其 `preferred_models` 中列出该 ID 的提供商。未知 ID 回退为纯基础默认规格。
2. **`models` 覆盖** — 用户在 `models` 中对该 ID 的局部配置。
3. **环境变量覆盖** — 进程环境变量优先级高于一切。

当仅传入 `--model` 时，Ante 寻找已认证且包含该模型的提供商，其次回退至无需模型列表的已认证提供商，最后回退至默认提供商。

当同时传入 `--provider` 与 `--model` 时，显式指定的提供商只要通过认证即优先胜出。

当均未指定时，Ante 自动按目录顺序选用第一个已认证提供商的 Heavy 旗舰模型（若均未认证则回退到 `local`）。

## 环境变量覆盖（Environment overrides） {#environment-overrides}
...
## 容错处理（Error handling） {#error-handling}

单个无效条目不会破坏整个配置文件。Ante 对 `catalog.json` 进行宽容反序列化：无法解析的条目会被跳过并打印启动提示，其余合法条目仍正常生效。仅当文件本身不是合法 JSON 时才会整体拒绝。若缺少 `catalog.json`，Ante 直接使用内置预设。
