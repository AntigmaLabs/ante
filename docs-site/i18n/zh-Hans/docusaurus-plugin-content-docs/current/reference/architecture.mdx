...
## 客户端与守护进程划分（Client-Daemon split） {#client-daemon-split}
...
- **Transport（传输层）** — 进程内客户端（TUI、无头模式）使用 Tokio 异步有界通道；外部客户端使用基于 stdin/stdout 的 JSONL 协议（`ante serve --stdio`）或 WebSocket 网络协议（`ante serve --ws`）。通过消息 ID 实现跨进程边界的链路追踪。

## 架构演进：从以会话为中心到以智能体为中心（Session-centric to Agent-centric） {#architecture-direction-session-centric-to-agent-centric}
...
## 大语言模型提供商（LLM providers） {#llm-providers}

Ante 具备跨提供商无关性。每个提供商均实现了发送提示词与接收流式响应的通用接口。流式传输过程中的偶发网络闪断会在轮次内自动重连；若流式调用在 5 分钟内未产生任何可用事件，将因超时报错而非无限挂起。该超时窗口支持按提供商配置 —— 可通过 [`stream_idle_timeout_secs`](/reference/catalog-reference#provider-fields) 覆盖。
...
### 内置工具清单（Built-in tools） {#built-in-tools}
...
### 工具过滤机制（Tool filtering） {#tool-filtering}
...
## 会话生命周期（Session lifecycle） {#session-lifecycle}
