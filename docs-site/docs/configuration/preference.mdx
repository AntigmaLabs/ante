...
Most of these settings can also be viewed and changed interactively with the `/config` command in the TUI — a searchable settings dialog with in-place toggles (tips, ambient predictions, short prompt, resize reflow, default permission mode, update channel) and shortcuts into the dedicated dialogs (theme, model, provider, status line, MCP servers, offline mode). Changes are persisted to `settings.json` immediately; settings consumed at startup, such as `resize_reflow`, are marked "(next session)" in the dialog and take effect on the next session.
...
| `short_prompt` | Use shorter built-in tool descriptions to shrink the system prompt, for new and resumed sessions. Off unless set to `true`; the `--short-prompt` CLI flag enables it for a single run |
