...
A repository can carry its own settings. When a session starts, Ante walks up from the session's working directory, and the nearest ancestor containing `.ante/settings.json` supplies a **project layer** over your user settings:
...
Everything else layers normally. `permissions.deny` and `permissions.ask` are *narrowing* rules, so a project can add them — that is the intended way to fence off destructive commands for a repository. `append_system_prompt`, `exclude_skills`, `model_effort`, `tools`, `auto_compact`, and the UI toggles all apply.
...
Project settings are read once, at session start. Editing the file mid-session has no effect until the next session.
...
4. CLI flags (`--model`, `--provider`, etc.)
