...
The built-in `bare` profile is file-less and starts from default settings with onboarding and ambient UI features disabled. It also disables skills, MCP servers, session saving, and auto-memory; explicit run flags such as `--enable-auto-memory` can still override the run behavior. Changes made in `/config` last only until that `bare` process exits.
