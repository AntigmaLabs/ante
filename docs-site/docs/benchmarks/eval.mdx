...
- **The build is the product.** Every eval runs against a pinned, publicly downloadable Ante release, the same binary `ante update` installs (e.g. `0.preview.43`). No eval-only branches, no benchmark-specific prompts.
