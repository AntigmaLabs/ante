...
Once a model is loaded, it is registered as the `local` provider in the catalog, so you can also switch to it from any session with `/providers` or target it with `--provider local`. See [mixing local and frontier](/local/overview#mixing-local-and-frontier).
...
| `extra_args` | Extra llama-server flags, appended to the launch command — see [Custom Engines](/local/custom-engines#tune-the-launch-command) |
