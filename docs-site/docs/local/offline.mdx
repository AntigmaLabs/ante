...
3. **Select a model** — Choose from:
   ...
   - **Running servers** — [attach](/local/custom-engines#attach-to-a-running-server) to a llama server already running on a local port
...
Once a model is loaded, it is registered as the `local` provider in the catalog, so you can also switch to it from any session with `/model` or target it with `--provider local`. See [mixing local and frontier](/local/overview#mixing-local-and-frontier).
