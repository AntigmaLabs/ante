...
When Ante starts its managed `llama serve` router, it owns that process and can
stop it. When Ante attaches to an external `llama-server`, stopping offline mode
only detaches Ante from the endpoint; the external server keeps running.
