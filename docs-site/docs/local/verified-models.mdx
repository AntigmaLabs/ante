...
description: "Curated local models tested with Ante, and the eval numbers behind them"
---

The **Verified models** list in the [offline mode](/local/offline) selector is curated: GGUF builds we have tested with Ante's agent loop, downloaded from Hugging Face with memory requirements known up front. Each model carries one of two statuses:
...
"Max context" is what the model supports; Ante caps the default context window at 32K to keep memory predictable, and you can raise it per model in [preferences](/local/offline#model-preferences) or with `ANTE_OFFLINE_CONTEXT`.
