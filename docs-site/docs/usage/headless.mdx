...
| `--short-prompt` | Use shorter built-in tool descriptions to shrink the system prompt |
| `--include-tools <TOOLS>...` | Only allow these tools (space-separated). Deprecated alias: `--allowed-tools` (still works, prints a warning) |
| `--exclude-tools <TOOLS>...` | Disallow these tools (space-separated). Deprecated alias: `--disallowed-tools` (still works, prints a warning) |
