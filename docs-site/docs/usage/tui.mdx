...
| `/model` | Select a model and connected provider |
...
Use `/goal <condition>` when you want Ante to keep driving the session until a concrete success condition is satisfied:

```text
/goal all tests pass and the docs build succeeds
```

Ante starts or continues work toward that condition, then evaluates the conversation after each turn. If the goal is not met, it sends a continuation turn with the evaluator feedback. The loop stops when the goal is met, judged unreachable, cleared, interrupted, or reaches its built-in iteration cap.

Run `/goal` with no arguments to show the current status, or `/goal clear` to stop the goal loop.
