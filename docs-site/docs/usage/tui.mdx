...
The [PTY tool](/reference/tools-reference#pty) drives a shared tmux session. `/pty` opens an embedded panel in the TUI showing that session live, and `Ctrl+P` switches focus between the chat input and the panel, so you can type into the same terminal the agent is driving.
