...
:::tip
The defaults assume a local llama-server. To use a hosted provider, pass `--provider` and `--model` explicitly (e.g. `ante gateway --provider anthropic --model claude-sonnet-5`).
:::
