...
:::tip
The defaults assume a local llama.cpp server or router. To use a hosted provider, pass `--provider` and `--model` explicitly (e.g. `ante gateway --provider anthropic --model claude-sonnet-5`).
:::
