...
| Model | Description | Context |
|---|---|---|
| `gemini-3.6-flash` | Latest Gemini Flash for agentic and multimodal workflows | 1M |
...
Run GGUF models locally via the built-in llama.cpp engine — no API key required. `local` is a wildcard provider; the available model depends on what the offline server has loaded. See [Offline Mode](/local/offline) for setup.
