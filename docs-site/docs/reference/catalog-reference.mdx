...
| `claude-fable-5` | Highest capability for demanding reasoning | 200K |
| `gpt-5.5` | Latest flagship GPT model | — |
| `gpt-5.5-pro` | Highest reliability on complex problems | — |
...
Run GGUF models locally via the built-in llama.cpp engine — no API key or internet required. `local` is a wildcard provider; the available model depends on what the offline server has loaded. See [Offline Mode](/usage/offline) for setup.
...
- [Offline Mode](/usage/offline) — running local GGUF models
