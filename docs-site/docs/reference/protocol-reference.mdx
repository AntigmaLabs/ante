...
Update the active session without restarting it (e.g. switch providers, models, or permission mode mid-session). Each field is optional; an absent field is left unchanged. A permission-mode change — what `Shift+Tab` cycling sends from the TUI — takes effect immediately, applying to the turn already in flight as well as later ones; a provider or model change applies on the next turn. A `title` change applies at once and is echoed back on the `SessionUpdated` event. Other session options must be set at `StartSession` time.

A `provider` change continues the conversation on the named catalog provider, keeping the session ID, messages, title, and permission state. `model` then names a model on that provider; when it is absent, the provider's default model is used. Naming the current provider is not a switch. The whole update is refused — answered with an `Error` instead of `SessionUpdated` — if the provider ID is unknown, or if a turn is in flight when a different provider is requested; the session is never re-routed to another provider. Interrupt the running turn first to switch away from a failing or rate-limited provider. Available since v0.2.5.
...
| Field | Type | Description |
|---|---|---|
| `provider` | `string?` | Catalog provider ID to switch to (e.g. `"openai"`), keeping the conversation. Refused with `Error` for an unknown ID or while a turn is in flight |
| `model` | `ModelSpec?` | New model specification to use. Carries the whole request, `effort` included: a set `effort` overrides the catalog default; unset fields resolve from the catalog. Resolved on the session's provider — the one `provider` names when both are set |
