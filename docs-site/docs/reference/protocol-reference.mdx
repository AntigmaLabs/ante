...
Manually trigger conversation compaction on the active session (what `/compact` sends). The daemon emits `CompactStart` and `CompactEnd` around the summarization.
