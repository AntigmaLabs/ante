...
| `permission_mode` | `PermissionMode?` | Tool approval mode: `"strict"`, `"auto"`, or `"yolo"`. `null` uses `strict` |
...
| `include_tools` | `string[]?` | Whitelist — only these tools are available. Also gates dynamically registered (MCP) tools |
| `exclude_tools` | `string[]?` | Blacklist — these tools are removed |
...
See the [Offline Mode](/usage/offline) page for the user-facing flows and the `--offline-model` CLI flag.
