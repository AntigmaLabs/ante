...
| `--short-prompt` | Use shorter built-in tool descriptions for this run, shrinking the system prompt. Persist the default with the `short_prompt` [setting](/configuration/preference). |
...
| `--include-tools <TOOLS>...` | Only allow these tools. Values are space-separated. `--tools` is a short alias; `--allowed-tools` is a deprecated alias that still works but prints a warning. |
| `--exclude-tools <TOOLS>...` | Remove these tools from the session. Values are space-separated. `--disallowed-tools` is a deprecated alias that still works but prints a warning. |
...
:::note
`serve` is session-agnostic, so it does not accept the session flags (`--model`, `--provider`, `--yolo`, `--system-prompt`, `--include-tools`, …). The connecting client configures those per session over the protocol.
:::
