...
| `--profile <NAME>` | Use `~/.ante/<name>.settings.json` as a whole-file replacement for `settings.json` for this run. An unknown name starts from defaults. The built-in file-less `bare` profile disables skills, MCP servers, session saving, auto-memory, and ambient UI features. Can also be set with `ANTE_PROFILE`. |
...
| `--no-skills` | Skip skill discovery for this run. No skills are advertised to the model or available as slash commands. |
