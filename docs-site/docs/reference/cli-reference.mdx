...
| `--profile <NAME>` | Use `~/.ante/<name>.settings.json` as a whole-file replacement for `settings.json` for this run. An unknown name starts from defaults. The built-in `bare` profile (seeded as a real file on first use) disables skills, MCP servers, session saving, auto-memory, and ambient UI features. Can also be set with `ANTE_PROFILE`. |
...
| `--tools <TOOLS>...` | Replace the default tool set with exactly these tools. Values are space-separated. Overrides the `tools` setting. Deprecated alias: `--allowed-tools` still works but prints a warning. |
...
| `--ws <ADDR>` | Serve the protocol over WebSocket on the given loopback socket address. Conflicts with `--stdio`. |
...
| `--version <VERSION>` | Install an exact release instead of the channel's newest release. Accepts values with or without a leading `v`, such as `v0.preview.82` or `0.preview.82`. Conflicts with channel arguments. |
| `-h`, `--help` | Print command help. |
...
ante update --version v0.preview.82
```
...
It needs no TTY and no API key: MCP warm-up is skipped and the session reaches readiness from the catalog alone. The report lists four checks — `home` (the Ante home and input history are writable), `catalog` (provider/model counts), `session` (the resolved `provider/model`), and `first frame` (the headless render) — followed by a total. Warnings (such as no authenticated provider, a skipped catalog entry, or a `settings.json` parse notice) are listed below the table. `doctor` exits non-zero if any hard check fails, including an unwritable Ante home, so it works in scripts.
