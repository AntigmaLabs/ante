...
Execute shell commands. Each call runs in a fresh non-interactive process — shell state does not persist between calls. Long-running commands (servers, watchers, slow builds) can be launched in the background; the agent gets back a handle under `~/.ante/run/jobs/<proc-id>/` with `status.json`, `stdout.txt`, and `stderr.txt`, and can stop the process by its pid. Live handles survive Ante restarts and are never collected at any age; a settled job's handle is collected at a later Ante startup, once its directory has sat unchanged for 24 hours. A trailing `&` on a background command is redundant and is stripped automatically. Cancelling a foreground Bash call terminates its whole process group, including child and grandchild processes. Requires approval.
...
ante --include-tools ViewImage "describe the screenshots in assets/"
